...
class _ImportExportPageState extends State<ImportExportPage> {
  ...
  String _buildExportText({
    ...
    required List<OperationalTask> categories,
  ...
  Future<void> _importData(
    ...
    AccountsLocalDataSource accountsDs,
  ) async {
    ...
    try {
      ...
      await planningDs.saveProjects(parsed.projects);
      await planningDs.saveActivities(parsed.activities);
      await planningDs.saveCategories(parsed.categories);
      await accountsDs.saveTransactions(parsed.transactions);
      await accountsDs.saveFinancialAccounts(parsed.accounts);
  ...
  List<OperationalTaskModel> _parseCategories(String csv) {
    final rows = _parseCsv(csv);
    if (rows.isEmpty) return [];
    final header = rows.first;
    return rows.skip(1).where((r) => r.length == header.length).map((r) {
...
class _ParsedExport {
  ...
  final List<OperationalTaskModel> categories;
